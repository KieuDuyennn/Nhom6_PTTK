...
CREATE TABLE PHONG (
    ...
    << << << < HEAD --- LoaiHinh        VARCHAR(50)     NOT NULL CHECK (LoaiHinh IN ('Ở ghép', 'Nguyên phòng')),
    == == == = >> >> >> > 7d028e39716799c43b838424f3c2840c3607ee8c TienThueThang NUMERIC(12, 0) NOT NULL CHECK (TienThueThang > 0),
    TrangThai VARCHAR(30) NOT NULL CHECK (
        TrangThai IN ('Còn giường trống', 'Hết giường', 'Đang giữ chỗ')
    ),
...
CREATE TABLE GIUONG (
    ...
    TinhTrang VARCHAR(20) NOT NULL CHECK (
        TinhTrang IN ('Chưa sử dụng', 'Đang sử dụng', 'Đang giữ chỗ')
    ),
...
CREATE TABLE PHIEU_YEU_CAU_XEM_PHONG (
    ...
    ThoiHanThue INT CHECK (ThoiHanThue > 0),
    -- số tháng
...
VALUES ('G01', 'P001', 1200000, 'Đang sử dụng'),
...
ORDER BY bang;
//
alter table public.thanh_toan drop constraint if exists thanh_toan_loaitt_check;
alter table public.thanh_toan
add constraint thanh_toan_loaitt_check check (
        loaitt::text = any (
            array [
      'Tiền cọc',
      'Tiền thuê',
...
alter table public.hop_dong drop constraint if exists hop_dong_trangthai_check;
alter table public.hop_dong
add constraint hop_dong_trangthai_check check (
        trangthai::text = any (
            array [
      'Mới',
      'Chưa xác nhận',
      'Đã ký xác nhận',
      'Chờ thanh toán đầu kỳ',
      'Đã thanh toán đầu kỳ',
      'Đã hủy',
      'Đã đăng ký lịch trả phòng',
      'Đã đối soát',
      'Chờ ký biên bản trả phòng',
