...
import { layChiTietYCTT, xacNhanYCTT, xoaYCTT } from '../services/lapYCTT.service';

const ChiTietYeuCauThanhToan = () => {
  ...
  useEffect(() => {
    // Lắng nghe sự kiện đóng tab/trình duyệt để tự động huỷ nếu đang ở trạng thái 'Mới'
    const handleUnload = (e) => {
      if (data && data.trangthai === 'Mới' && !confirming && !isDeleting) {
        const token = localStorage.getItem('token');
        fetch(`http://localhost:5000/api/yeu-cau-thanh-toan/${maTT}`, {
          method: 'DELETE',
          keepalive: true,
  ...
  }, [maTT, data, confirming, isDeleting]);
  ...
  const handleQuayLai = async () => {
    if (data?.trangthai === 'Mới') {
      try {
        setIsDeleting(true);
        await xoaYCTT(maTT);
      } catch (e) {
        console.error('Lỗi khi xoá YCTT nháp:', e);
      } finally {
        setIsDeleting(false);
    ...
    <MainLayout>
      ...
      <div className="mb-8">
        <button
          ...
          disabled={isDeleting}
          className="flex items-center gap-2 text-[14px] text-gray-500 hover:text-navy transition-colors mb-4 font-medium disabled:opacity-50"
        >
          {isDeleting ? (
          ...
          {isDeleting ? 'Đang hủy yêu cầu...' : 'Quay lại danh sách'}
        </button>
...
export default ChiTietYeuCauThanhToan;
